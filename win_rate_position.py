...
class HeroMaxLikelihood:
    ...
    def likelihood(self, heroes_idx, max_log_like):
        ...
        # Original
        log_like = 0
        for hero, position in zip(heroes_idx,
                                  ["P{}".format(t + 1) for t in range(5)]):
...
def main():
    ...
    stmt = "SELECT match_id, radiant_heroes, dire_heroes, radiant_win FROM " \
           "dota_matches LIMIT 5000"
    cursor.execute(stmt)
